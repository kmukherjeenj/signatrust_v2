...
export class SolanaDIDProvider {
  ...
  constructor(options: { rpcUrl: string; programId: string }) {
    if (!options.rpcUrl) {
  ...
  async createIdentifier(payerKeypair: Keypair): Promise<{ did: string; publicKey: string }> {
    try {
  ...
  async updateIdentifier(did: string, payerKeypair: Keypair, updateData: any): Promise<string> {
    // Implement update logic
    // This should update the on-chain data and return the transaction signature
    throw new Error('Method not implemented.');
  }

  async deactivateIdentifier(did: string, payerKeypair: Keypair): Promise<string> {
    try {
      const publicKey = new PublicKey(did.split(':')[2]);
      const transaction = new Transaction().add(
  ...
  async getDIDAccount(did: string): Promise<{ controller: string; publicKey: string } | null> {
    try {
      const publicKey = new PublicKey(did.split(':')[2]);
      const accountInfo = await this.connection.getAccountInfo(publicKey);
      if (accountInfo) {
