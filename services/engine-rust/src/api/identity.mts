...
export const createIdentity = async () => {
  try {
    ...
    const { proof, publicSignals } = await zkProofService.generateAuthProof(
      publicKey,
      signature,
      message
    );
...
async function signMessage(message: string, publicKeyString: string): Promise<string> {
  try {
...
export const getIdentity = async (did: string) => {
  try {
...
export const deleteIdentity = async (did: string) => {
  try {
...
export const updateIdentity = async (did: string, updateData: any) => {
  try {
...
export const login = async (did: string, proof: any, publicSignals: any) => {
  try {
