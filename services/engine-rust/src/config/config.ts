...
const parseJsonEnv = (key: string, defaultValue: any): any => {
  const value = process.env[key];
  if (value === undefined) {
    console.warn(`Warning: ${key} is not set in the environment variables. Using default value.`);
    return defaultValue;
...
const loadKeypairFromFile = (path: string): Keypair | null => {
  try {
...
const config = {
  solana: {
    ...
    payerKeypair: null as Keypair | null, // Initialize as null
  },
  zkProof: {
