...
export class SecureEnclaveService {
  private enclave: Map<string, Buffer> = new Map();

  async storeKeyPair(did: string, keyPair: Keypair): Promise<void> {
    try {
  ...
  async retrieveKeyPair(did: string): Promise<Keypair | null> {
    try {
  ...
  async signChallenge(did: string, challenge: string): Promise<Uint8Array | null> {
    try {
  ...
  private encryptKey(key: Uint8Array): Buffer {
  ...
  private decryptKey(encryptedKey: Buffer): Uint8Array {
