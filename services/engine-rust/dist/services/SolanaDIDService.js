...
export class SolanaDIDService {
    ...
    async signDocument(did, documentId, signature) {
        try {
    ...
    async updateDID(did, payerKeypair, updateData) {
        const publicKey = new PublicKey(did.split(':')[2]);
    ...
    async deactivateDID(did, payerKeypair) {
        const publicKey = new PublicKey(did.split(':')[2]);
        const transaction = new Transaction().add(
    ...
    async storeDocumentHash(did, documentHash) {
        const keypair = this.getKeypairFromDID(did);
